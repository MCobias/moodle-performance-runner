 ...
 *
 * @package moodle-performance-runner
...
class report {
     ...
     */
    const RUNS_RELATIVE_PATH = 'jmeter_data/runs/';
     ...
     * @param string $threadgroupname
     * @return bool Whether runs are comparable or not.
     */
    public function parse_runs(array $timestamps, $threadgroupname) {
        ...
        foreach ($timestamps as $timestamp) {
            ...
            $run = new test_plan_run($timestamp.'_'.$threadgroupname);
            $run->parse_results();
            $this->runs[] = $run;
        }
     ...
     */
    public function make(array $timestamps, $threadgroupname) {

        // They come from the form in the opposite order.
        krsort($timestamps);

        // Gets the runs data and checks that it is comparable.
        if (!$this->parse_runs($timestamps, $threadgroupname)) {
        ...
        if ($dh = opendir($dir)) {
            while (($filename = readdir($dh)) !== false) {
                ...
                if ($filename != '.' && $filename != '..' &&
                    ...
                    $runfiles[$filename] = new test_plan_run($filename);

                    // Get the params for filtering.
                    foreach (test_plan_run::$runparams as $param => $name) {

                        // In case some runs misses vars.
                        if (!empty($runfiles[$filename]->get_run_info()->{$param})) {
                            $value = $runfiles[$filename]->get_run_info()->{$param};
                        } else {
                            $value = 'Unknown';
                        }
                    ...
                    if (!empty($_GET['filters'])) {
                        foreach ($_GET['filters'] as $param => $filteredvalue) {
                            // In case some runs misses vars.
                            if (!empty($runfiles[$filename]->get_run_info()->{$param})) {
                                $runvar = $runfiles[$filename]->get_run_info()->{$param};
                            } else {
                                $runvar = 'Unknown';
                            }
                            // Ensure it still exists.
                            if (!empty($filteredvalue) && !empty($runfiles[$filename]) &&
                                    $filteredvalue != $runvar) {
                                unset($runfiles[$filename]);
                                break;
                            }
                        }
                    }
